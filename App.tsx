...
const App: React.FC = () => {
    ...
    const handleGenerate = useCallback(async (idea: string) => {
        ...
        try {
            ...
            const imagePrompts = [
                { type: 'banner', prompt: `An epic dark fantasy landscape or scene that could serve as a banner for a collection called "The Demon Codex". Tones: ${textData.tone}. Cinematic, dramatic, gothic, high contrast. Do not include any text.`, aspectRatio: '16:9' as const },
                ...textData.cards.map((card, index) => ({
                    type: `card-${index}`,
                    prompt: `${card.title}, ${card.prompt.replace(/\n/g, ', ')}, cinematic reference, dark fantasy art, high detail`,
            ...
            for (let i = 0; i < imagePrompts.length; i++) {
                ...
                setLoadingMessage(`Generating visual ${i + 1} of ${imagePrompts.length}: ${p.type}...`);
                const imageUrl = await geminiService.generateImage(p.prompt, p.aspectRatio);
                ...
                {!isLoading && !generatedData && (
                     <div className="flex flex-col items-center justify-center min-h-[80vh]">
                        <div className="text-center mb-8 animate-fade-in">
                            <h1 className="text-6xl md:text-8xl font-extrabold tracking-tighter text-transparent bg-clip-text bg-gradient-to-br from-[#f5a16f] to-[#b14a38]" style={{filter: 'drop-shadow(0 0 10px #8d1a1a)'}}>
                                The Demon Codex
                            </h1>
                            <p className="text-xl text-[#9aa0a6] mt-4">Dark Fantasy Thumbnail Vault</p>
                        </div>
                        <GeneratorForm
