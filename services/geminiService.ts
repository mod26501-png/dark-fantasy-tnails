...
const responseSchema = {
  ...
  properties: {
    archetype: {
      type: Type.STRING,
      description:
        "A single, powerful archetype that captures the theme (e.g., Warlord, Scholar, Rogue).",
    },
    tone: {
      type: Type.STRING,
      description:
        "Two or three descriptive words for the tone (e.g., Gothic, Dramatic).",
    },
    use: {
      type: Type.STRING,
      description:
        "The intended use case for these images (e.g., Cinematic Image Generation).",
    },
    cards: {
      type: Type.ARRAY,
      ...
      items: {
        ...
        properties: {
          ...
          title: {
            type: Type.STRING,
            description: "A short, evocative title for the card.",
          },
          prompt: {
            type: Type.STRING,
            description:
              "A multi-line string with 7 lines detailing Subject, Lens, Lighting, Style, Color, Composition, and Constraints.",
          },
          caption: {
            type: Type.STRING,
            description:
              "A brief, descriptive caption for the image generated from the prompt.",
          },
          tags: {
            type: Type.ARRAY,
            description:
              "An array of 2-4 single-word lowercase tags for filtering (e.g., character, creature, cathedral, symbol).",
    ...
    negativePrompts: {
      type: Type.ARRAY,
      description:
        "An array of 5 short strings for a 'Negative Prompt Overlay' to avoid clichés.",
      items: { type: Type.STRING },
    },
    remixSuggestions: {
      type: Type.ARRAY,
...
export const generatePromptSet = async (
  ...
  & Omit<
    GeneratedData,
    "mainTitle" | "logoImageUrl" | "bannerImageUrl" | "cards"
  >
  & { cards: Omit<PromptCard, "imageUrl">[] }
> => {
  const prompt = `
...
export const generateImage = async (
  ...
  try {
    ...
    if (
      ...
      console.warn(
        "Image generation blocked, likely by safety filters for prompt:",
        prompt,
      );
